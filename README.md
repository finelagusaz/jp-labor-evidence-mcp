...
- **安衛通達取得** — 安衛通達の本文を取得
- **内部索引優先** — 候補検索は internal index を優先し、必要時のみ upstream へ fallback
...
| `diff_revision` | 同一法令の改正前後に相当する 2 つの e-Gov law_id 上で同一条文を比較し、構造化 diff を返す |
...
検索系ツールは `used_index` と `index_freshness` を返します。通常は internal index を優先し、index に候補がない場合だけ upstream 検索へ fallback します。
...
→ `diff_revision(base_law_id="old-law-id", head_law_id="new-law-id", article="32")`

`diff_revision` は同一法令の同じ条番号について、比較元と比較先の本文を `diff_chunks` として返します。異なる法令同士の比較は受け付けません。
