...
> [!NOTE]
> **何が返るか**: e-Gov・厚生労働省法令等データベース・JAISH から取得した **法令条文と通達本文の原文**、**出典 URL**、**取得日時・検索経路などの監査 metadata** を、構造化レスポンス (`structuredContent`) として返します。Markdown テキストは表示用の補助です。
>
> **役割**: 上位の LLM クライアントに「根拠と出典」をそのまま渡すことです。原文を加工せず一次情報のまま返すことで、引用ハルシネーションや出典不明の回答を抑えます。法的結論や実務判断はサーバー側では行いません。
