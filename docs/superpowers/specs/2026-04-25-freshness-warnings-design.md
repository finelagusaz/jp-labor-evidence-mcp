...
1. [#1 Freshness warnings opt-out via env var](https://github.com/finelagusaz/jp-labor-evidence-mcp/issues/1) — `LABOR_LAW_MCP_SUPPRESS_FRESHNESS_WARNINGS=1` で意図的に古い bundle を使う利用者向け
2. [#2 Display dates in freshness warnings in JST](https://github.com/finelagusaz/jp-labor-evidence-mcp/issues/2) — 現状の YYYY-MM-DD は UTC 由来で最大 1 日のずれ。JST 変換で正確化
3. [#3 Expose freshness status via MCP resource](https://github.com/finelagusaz/jp-labor-evidence-mcp/issues/3) — `mcp://jp-labor-evidence-mcp/status` による on-demand 状態照会
4. [#4 Calendar-aware bundled age threshold](https://github.com/finelagusaz/jp-labor-evidence-mcp/issues/4) — 直近 4/1・10/1 を跨いだら即警告する閾値代替
